...
void DataProcessor::readData() {
    ...
    if (data.size() != 5) {
        throw std::runtime_error("Файл должен содержать ровно 5 значений");
    }
}
...
std::vector<double> DataProcessor::getSensitivity() 
{
    for (int i = 1; i < 6; i++) 
    {
        sensitivity.push_back(10 * i / (2 * sqrt(2) * data[i - 1]));
    }
    return sensitivity;
}
